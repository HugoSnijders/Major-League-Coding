...
new <-  read_csv("Life_Expectancy.csv", 
                                   col_types = cols(American = col_character(), 
                                                    Athlete = col_character(), BirthYear = col_number(), 
...
new.football <- new.football %>%
  group_by(BirthYear) %>%
  mutate(Avg.exp = (mean(Age, na.rm = TRUE)))
new.nonathletes <- new.nonathletes %>%
  group_by(BirthYear) %>%
  mutate(Avg.exp = (mean(Age, na.rm = TRUE)))
...
ggplot() +
  ...
  geom_point(data = new.nonathletes, mapping = aes(x=BirthYear, y=Avg.exp, colour='Non-Athletes'))
